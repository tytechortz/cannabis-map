...
colorado_data = '//Users/jamesswank/Python_projects/cannabis-map/Colorado_County_Boundaries.geojson'
geo = pd.read_json(colorado_data)
...
i = 0
while i < L:
    names.append(geo['features'][i]['properties']['COUNTY'])
    i += 1

county_geojson={'type': 'FeatureCollection',
                'features': []}

for k in range(L):
    county_geojson['features'].append(geo['features'][k])
